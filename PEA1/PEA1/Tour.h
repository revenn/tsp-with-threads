...
public:
	...
	Tour(Cities c) { cities.push_back(0); mileage = 0; miasta = c; }
	...
	unsigned int mileage_delta(unsigned int c) {
		...
		unsigned int test_mileage = miasta.mileage_table[last][c];

		// If this would be our last city, add the mileage
		// to return home
		if (cities.size() + 1 == miasta.rozmiar) {
			test_mileage += miasta.mileage_table[c][0];
		}
		return test_mileage;
	}
